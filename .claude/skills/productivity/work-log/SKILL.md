...
name: work-log
description: |
  현재 브랜치의 작업 내용을 분석하여 Confluence에 구조화된 작업 로그를 자동 생성합니다. 비개발자도 쉽게 읽을 수 있도록 SVG 다이어그램·패널 매크로·status 배지·표(Table)로 풍부하게 시각화합니다.
...
비개발자도 쉽게 읽을 수 있도록 **SVG 다이어그램 · info/tip/note 패널 · status 배지로 풍부하게 시각화**하고, 표(Table)를 적극 활용합니다.
...
작업 흐름 · 상태 전이 · 아키텍처 · 분기 로직은 **SVG 다이어그램 2~4개**로 시각화한다. 전체 개요도 1개는 필수.

Read 도구로 `references/visual-guide.md`를 읽어 SVG 작성 규칙(색상 팔레트 · 박스/화살표 골격 · 다이어그램 유형)과 Confluence 삽입 방식을 따른다. 외부 다이어그램 스킬에 의존하지 않고 직접 작성한다.
...
> **🔑 페이지 레이아웃 규칙 (기본: wide)** — 모든 work-log 페이지는 다음 두 규칙을 무조건 적용한다.
> 1. **본문 전체를 wide breakout 으로 wrap** — 페이지가 기본 폭으로 좁게 보이는 것을 방지.
>    ```xml
>    <ac:layout>
>      <ac:layout-section ac:type="single" ac:breakout-mode="wide">
>        <ac:layout-cell>
>          <!-- 본문 콘텐츠 -->
>        </ac:layout-cell>
>      </ac:layout-section>
>    </ac:layout>
>    ```
> 2. **모든 표에 `data-layout="wide"` 속성** — 표가 본문 영역에서 가능한 한 넓게 표시되도록.
>    ```xml
...
>    ```
>
> 3. **SVG 다이어그램 + 시각 매크로** — 작업 흐름/상태 전이를 SVG 2~4개로 시각화하고, `info`/`tip`/`note` 패널과 `status` 배지로 글 블록을 분해한다. SVG는 attachment + `<ac:image>`로 삽입한다(HTML 매크로 금지). 상세 규칙은 `references/visual-guide.md`.
>
> 더 넓게 표시해야 하는 와이드 다이어그램/대형 표는 `breakout-mode="full-width"` 또는 `data-layout="full-width"` 사용. 단 일반 표는 `wide` 가 기본.
...
- 위 § 페이지 레이아웃 규칙(wide breakout + table wide) 적용 필수
...
- 위 § 페이지 레이아웃 규칙(wide breakout + table wide + 시각 매크로) 적용 필수

**5-4. SVG 다이어그램 첨부 (필수)**

Step 3에서 만든 SVG를 페이지에 첨부한다. `references/visual-guide.md` § 2(attachment 방식)의 curl 절차를 따른다.
- 본문에는 `<ac:image><ri:attachment ri:filename="..." /></ac:image>`로 삽입한다. **HTML 매크로(`ac:name="html"`)는 Confluence Cloud에서 렌더되지 않으므로 사용 금지.**
- 신규: 페이지 생성(본문에 `ac:image` 참조 포함) → SVG 첨부. 기존: SVG 첨부 → 본문 PUT.
- 작업 후 임시 SVG 파일은 삭제한다(첨부본은 페이지에 보존됨).
...
- 처리 흐름은 글보다 다이어그램이 직관적이므로 SVG로 시각화한다 (작성 규칙은 `references/visual-guide.md`)
- 긴 글 블록은 `info`/`tip`/`note` 패널과 `status` 배지로 분해해 한눈에 읽히게 한다
...
표는 항상 `<table data-layout="wide">` 로 작성한다 (Step 5 § 페이지 레이아웃 규칙 참조). 좁은 기본 폭은 가독성을 떨어뜨려 표의 정보 전달력을 약화시키기 때문이다.
...
- **모든 페이지는 wide 레이아웃을 기본으로 한다** — Step 5 § 페이지 레이아웃 규칙 참조 (좁은 기본 폭이 표/다이어그램 가독성을 떨어뜨리므로)
