...
1. `references/template.html`을 산출 경로로 복사한다. (라이트/다크 auto · 카드형 대시보드 디자인 시스템 내장)
2. `<title>`, 그라디언트 헤더(`.doc-header` — 제목·설명·메타)를 채운다.
3. 수치가 있으면 헤더 아래 `.stats-grid` 통계 카드를 둔다(없으면 블록 삭제). 각 섹션은 `<div class="card">`로 작성하고 `<h2>`에 아이콘을 붙인다. 긴 보조 내용은 `<details>`로 접는다.
4. 비교·체크리스트·상태는 `<table>`/`.badge`(ok/warn/err/info)로 표현한다.
