# Mermaid Diagram Syntax 레퍼런스

20가지 다이어그램 유형별 문법과 예제.

---

## 1. Flowchart (플로우차트)

프로세스 흐름, 의사결정, 시스템 구조 표현에 가장 많이 사용.
...
**차트 유형:** `bar` (막대), `line` (선) — 둘 다 함께 사용 가능
...
**아이콘 유형:** `cloud`, `server`, `database`, `disk`, `internet`
...
**문법:** `source,target,value` CSV 스타일
...
**관계:** `contains`, `derives`, `satisfies`, `verifies`, `refines`, `traces`
