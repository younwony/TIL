...
`ready-for-agent`로 결정되고 메인테이너 승인 시 새 Track을 자동 생성할 수 있다 (`.claude/tracks/{이슈키}-{설명}/`). `1_REQ-SNAPSHOT.html`에 이슈 본문 + AGENT-BRIEF 저장.

산출 HTML 문서는 `html-doc` 스킬의 규칙을 따라 자체 완결 HTML로 작성한다. 산출 HTML 문서에는 html-doc 스킬의 시각화 가이드에 따라 인라인 SVG 다이어그램을 1개 이상 포함한다 (예: 이슈 상태 전이 흐름도, 영향 모듈 구조도 등).
