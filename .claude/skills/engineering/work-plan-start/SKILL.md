...
description: WORK-SPEC.md 작업 명세서를 기반으로 단계별 실제 구현을 수행한다. Solo/Standard/Coordinator 모드 자동 선택. 내부에서 tdd, diagnose, test-generator atomic skill/agent를 조합 호출하는 wrapper. "/work-plan-start", "작업 시작", "WORK-SPEC 실행", "명세서 기반 작업", "계획대로 구현", "스펙 실행" 요청 트리거.
---

# Work Plan Start (Atomic Wrapper)

`2_WORK-SPEC.md`를 입력으로 단계별 구현을 진행하고 `4_PLAN.md` / `5_ARCHITECTURE.md` / `6_SPEC.md`를 산출한다.
...
**Requires**: 현재 Active Track의 `*_WORK-SPEC.md` 존재. 없으면 `/work-plan` 먼저.

## 모드 자동 선택

WORK-SPEC.md의 예상 변경 파일 수에 따라:
...
- Active Track의 `2_WORK-SPEC.md` 존재 확인 → 없으면 사용자에게 안내 후 종료
- `3_FEATURE-CHECKLIST.md` 존재 확인 (없어도 진행 가능, 단 경고)
- `metadata.json`의 `current_phase` 확인 → 재개 가능 여부 판단
- `.claude/CONTEXT.md` 읽기 → 도메인 어휘 인지

### Step 2 — Phase 계획

WORK-SPEC.md를 파싱하여 Phase 목록 추출. 보통 4~6개. 각 Phase는:

- 제목 + 변경 인터페이스 (AGENT-BRIEF 양식)
- 의존성 (이전 Phase 산출물 사용 여부)
- 검증 방법 (테스트 명령, 동작 확인)

`4_PLAN.md`에 Phase 진행률 표 작성:
...
3. 검증 (테스트 실행)
4. `4_PLAN.md` 상태 갱신 (in_progress → completed)
...
- 즉시 처리: 현재 Phase에 통합
- 후속 처리: WORK-SPEC.md "Out of Scope"에 추가 + 새 Track 후보

### Step 6 — 산출물 생성

구현 완료 시:

- `5_ARCHITECTURE.md` — 시스템 아키텍처, 데이터 흐름, SVG 다이어그램 (svg-diagram 또는 mermaid-diagram skill)
- `6_SPEC.md` — 기능 명세, 사용자 인터페이스, 핵심 로직 다이어그램
- `4_PLAN.md` — 모든 Phase 상태 completed로 갱신
...
[Phase 3 — UI]
   ...
   └─ 검증
   ↓
5_ARCHITECTURE.md + 6_SPEC.md 생성
4_PLAN.md 완료 상태 갱신
```

## 관련

- `/work-plan` — 본 skill의 입력 (WORK-SPEC.md) 생성
