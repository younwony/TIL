---
name: svg-diagram
description: 직접 SVG 코드를 작성하여 다이어그램을 생성합니다. 구조도, 비교 다이어그램, 테이블 형식 등 정교한 레이아웃이 필요할 때 사용하세요.
---
...
| 상황 | SVG 직접 생성 적합 |
|------|-------------------|
| **계층 구조** | OSI 7계층, 조직도, 폴더 구조 |
| **구조도** | 아키텍처, 시스템 구성, 네트워크 토폴로지 |
| **데이터 구조** | 패킷/프레임 구조, 메모리 레이아웃 |
| **테이블 형식** | 비교표, 스펙 정리 |
| **비교 다이어그램** | 좌우 비교, 전후 비교 |
