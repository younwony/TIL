...
name: cs-sync
description: |
  CS-GUIDE.md가 업데이트되었을 때, 기존 CS 문서들을 새 가이드 형식에 맞춰 일괄 동기화합니다. 누락 섹션 추가, 섹션 순서 정렬, 난이도 표기 형식 통일을 자동으로 처리합니다.
  "CS 동기화", "CS 싱크", "가이드 반영", "템플릿 동기화", "{카테고리} 동기화" 요청에 트리거됩니다.
  CS-GUIDE.md를 수정했거나 문서 형식이 일관되지 않다면 이 스킬을 실행하세요.
---
