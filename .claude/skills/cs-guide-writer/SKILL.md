...
하나의 메시지에서 **모든 작업을 동시에** 시작한다:

**[다이어그램] 분할 디스패치 규칙**

> **⚡ 핵심 원칙: 다이어그램 1개 = 에이전트 1개. 무조건 분할한다.**

| 다이어그램 수 | 디스패치 방식 | 예상 시간 |
|-------------|-------------|----------|
| 0개 | 다이어그램 에이전트 없음 | 0초 |
| 1개 | `cs-diagram-generator` × 1 (백그라운드) | ~30초 (Mermaid) / ~90초 (SVG) |
| 2개 | `cs-diagram-generator` × 2 (백그라운드, 각각 1개씩) | ~30초 (병렬) |
| 3개+ | `cs-diagram-generator` × N (백그라운드, 각각 1개씩) | ~30초 (병렬) |

각 에이전트에게 **다이어그램 1개만** 할당한다:
...
> **⚡ Mermaid 우선 정책**: 속도 최적화를 위해 **Mermaid를 기본 방식**으로 사용한다.
> SVG 직접 생성은 Mermaid로 표현이 불가능한 경우에만 사용한다.
...
#### Mermaid로 대체 가능한 경우 (SVG 직접 → Mermaid)

| 기존 SVG 직접 | Mermaid 대체 방식 |
|--------------|------------------|
| 계층 구조도 | `graph TD` (트리 구조) |
| 비교표/매트릭스 | `quadrantChart` 또는 테이블은 마크다운으로 |
