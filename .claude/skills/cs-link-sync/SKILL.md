...
name: cs-link-sync
description: |
  CS 문서의 깨진 마크다운 링크를 자동으로 찾아 수정합니다. README 목차, 연관 문서, 선수 지식 링크까지 전체 검사합니다.
  "링크 동기화", "깨진 링크 수정", "링크 체크", "링크 검증", "{카테고리} 링크 수정" 요청에 트리거됩니다.
  파일명이나 폴더 구조를 변경한 후에는 반드시 이 스킬로 링크 정합성을 검증하세요.
---
...
CS 문서가 늘어나면 파일명 변경, 폴더 이동이 빈번해집니다. 마크다운 링크는 상대 경로 기반이라 파일 이동 시 자동으로 갱신되지 않습니다.
깨진 링크는 문서 간 연결을 끊어 학습 흐름을 방해합니다. 이 스킬이 전체 문서를 스캔하여 깨진 링크를 자동 감지하고, 유사 파일명 매칭으로 수정 후보를 제안합니다.
