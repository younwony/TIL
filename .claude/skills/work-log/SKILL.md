...
- 이 작업이 **무엇을 하는지** (기능 설명)
- **왜 필요한지** (배경과 목적)
- **어떻게 동작하는지** (처리 흐름 — SVG 다이어그램과 표로 표현)
...
1. **SVG 직접 생성 우선**: `svg-diagram` 스킬을 사용하여 정교한 다이어그램을 생성한다. 그라디언트, 드롭섀도우 등 시각 효과를 적용한다. Confluence HTML 매크로(`ac:structured-macro ac:name="html"`)로 인라인 삽입한다.
2. **ASCII 폴백**: 간단한 트리 구조, 폴더 계층 등은 Confluence 코드 블록 내 ASCII로 표현 가능하다. `references/ascii-guide.md`를 참조한다.

SVG → Confluence 삽입 형식:
```xml
...
- 처리 흐름은 글보다 다이어그램이 직관적이므로 시각화한다 (SVG 우선, 간단한 구조는 ASCII 폴백)
