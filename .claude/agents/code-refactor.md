...
아래 조건을 **모두** 충족해야 Sonnet Advisor에게 리팩토링 전략을 위임한다:
1. 단순 패턴 수정이 아닌 클래스 구조 변경 (레이어 분리, 의존성 역전, DTO 도입 등) **AND**
2. 5개 이상의 파일에 걸친 연쇄적 변경 **AND**
3. 리팩토링 순서에 따라 컴파일/테스트 깨질 위험이 있는 경우

단일 파일 리팩토링, 패턴 치환, 3개 미만 파일 변경은 Advisor 없이 haiku가 직접 처리한다.
