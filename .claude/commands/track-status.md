...
3. WORK-SPEC.html와 Track이 자동 생성됩니다

### 커맨드 요약

| 단계 | 커맨드 | 설명 |
|------|--------|------|
| 1. 계획 | `/work-plan [path]` | 요구사항 분석 → WORK-SPEC.html + Track 생성 |
| 2. 구현 | `/work-plan-start [path]` | WORK-SPEC.html 기반 코드 구현 |
| 3. 테스트 커버리지 | `/test-coverage-check` | 변경 파일 커버리지 분석 + 누락 테스트 자동 생성 |
| 4. 코드 리뷰 | `/self-review` + `/simplify` | 자체 코드 리뷰 + 코드 품질 개선 |
| 5. QA 시나리오 | `/qa-scenario` | 변경사항 분석 → QA-SCENARIOS.html 생성 |
...
각 Track의 `*_WORK-SPEC.html`에서 "파일 변경 목록" 섹션을 읽어 변경 예상 파일 수를 카운트한다.
...
| 체크 항목 | 판별 방법 | 상태 |
|----------|----------|------|
| 계획 수립 | `*_WORK-SPEC.html` 존재 | ✅/❌ |
| 기능 체크리스트 | `*_FEATURE-CHECKLIST.html` 존재 | ✅/❌ |
| 구현 진행 | `*_PLAN.md`의 체크박스 진행률 | N% |
| 테스트 커버리지 | 마지막 `/test-coverage-check` 실행 여부 (git log에서 테스트 파일 변경 확인) | ✅/❌/⏳ |
| 셀프 리뷰 | `*_SELF-REVIEW.html` 존재 | ✅/❌ |
| QA 시나리오 | `*_QA-SCENARIOS.html` 존재 | ✅/❌ |
| 아키텍처 문서 | `*_ARCHITECTURE.html` 존재 | ✅/❌ |
| 기능 명세 | `*_SPEC.md` 존재 | ✅/❌ |

## 2단계: 진행률 계산

각 Track의 `*_PLAN.md` (예: `3_PLAN.html`)를 읽어 진행률을 계산한다:
...
| 단계 | 상태 | 다음 액션 |
|------|------|----------|
| 1. 계획 (`*_WORK-SPEC.html`) | ✅ | - |
| 2. 구현 (`*_PLAN.md`) | 🔄 65% | `/work-plan-start` 계속 |
| 3. 테스트 커버리지 | ⏳ | `/test-coverage-check` |
| 4. 셀프 리뷰 (`*_SELF-REVIEW.html`) | ⏳ | `/self-review` |
| 5. QA 시나리오 (`*_QA-SCENARIOS.html`) | ⏳ | `/qa-scenario` |
...
| # | 단계 | 파일 | 상태 |
|---|------|------|------|
| 1 | 계획 수립 | `*_WORK-SPEC.html` | ✅/❌ |
| 2 | 기능 체크리스트 | `*_FEATURE-CHECKLIST.html` | ✅/❌ |
| 3 | 구현 | `*_PLAN.md` | 🔄 N% |
| 4 | 테스트 커버리지 | (테스트 파일 존재 여부) | ✅/❌/⏳ |
| 5 | 셀프 리뷰 | `*_SELF-REVIEW.html` | ✅/❌ |
| 6 | QA 시나리오 | `*_QA-SCENARIOS.html` | ✅/❌ |
| 7 | 아키텍처 문서 | `*_ARCHITECTURE.html` | ✅/❌ |
| 8 | 기능 명세 | `*_SPEC.md` | ✅/❌ |
