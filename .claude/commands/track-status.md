...
| 코드 리뷰 완료, QA Phase 미시작 | → 코드 리뷰 완료. `/qa-scenario`로 QA 시나리오를 생성하세요 |
| QA 시나리오 생성 완료, 브라우저 QA 미실행 | → QA 시나리오가 준비되었습니다. `/browser-debug`로 브라우저 QA를 실행하세요 |
| 브라우저 QA 완료, 사용자 QA 대기 중 | → 수동 QA를 완료한 후 `/pr`로 PR을 생성하세요 |
