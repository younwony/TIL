...
> Codex Plugin이 설치된 경우 **Skill 도구**(`codex:rescue`)로 실행. Plugin 미설치 시에만 CLI fallback.
> Gemini 크로스 체크와 독립적이므로 **가능한 경우 병렬로 실행**한다.
> **⚠️ 절대 Bash로 `codex exec -`를 사용하지 않는다. 반드시 Skill 도구 우선.**
...
**Plugin 설치됨 → Skill 도구로 호출 (Bash가 아님!):**
```
Skill 도구 호출:
  skill: "codex:rescue"
...
- 실패 시 "Codex 크로스 체크에 실패했습니다." 안내 후 계속 진행
