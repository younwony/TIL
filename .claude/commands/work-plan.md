...
req.md 요구사항 파일을 분석하여 WORK-SPEC.html 작업 명세서를 생성해줘.
생성된 WORK-SPEC.html는 `/work-plan-start`의 입력으로 사용됩니다.
...
## 3단계: WORK-SPEC.html 초안 생성

`.claude/docs/`(또는 req.md와 같은 디렉토리)에 `WORK-SPEC.html`를 생성한다.
...
WORK-SPEC.html 초안 생성 후, Gemini CLI와 Codex CLI를 활용하여 작업 계획을 크로스 체크한다.
...
WORK-SPEC.html와 req.md 내용을 Gemini에게 파이프로 전달하여 검증한다:
...
- 파일 경로는 req.md와 WORK-SPEC.html의 **실제 경로**를 사용한다
- Gemini CLI 실행이 실패하면 "Gemini 크로스 체크에 실패했습니다." 안내 후 계속 진행

### Codex 크로스 체크

> **Bash CLI 단독 사용** (Plugin Skill 사용 금지 — CLAUDE.md "Codex 협업" 섹션 참조).
> Gemini 크로스 체크와 독립적이므로 **가능한 경우 병렬로 실행**한다.
> **Bash 도구 호출 시 `timeout: 240000ms` (4분) 필수 명시**.

**CLI 호출 (Bash, timeout: 240000ms):**
```bash
(echo "검증 요청:" && cat {req.md 경로} && echo -e "\n---\n" && cat {WORK-SPEC.html 경로}) | codex exec - 2>&1 || echo "CODEX_FAIL"
```

- 4분 초과 시 자동 종료, `CODEX_FAIL` 검출 시 "Codex 크로스 체크에 실패했습니다." 안내 후 계속 진행

### 피드백 반영

1. Gemini/Codex 피드백 중 **❌ 필수 반영** 항목은 WORK-SPEC.html에 즉시 반영
2. **⚠️ 권장** 항목은 Claude가 타당성을 판단하여 선택적 반영
3. **💡 참고** 항목은 WORK-SPEC.html 하단에 "크로스 체크 참고사항" 섹션으로 추가
4. 피드백 반영 후 WORK-SPEC.html를 최종 업데이트

### WORK-SPEC.html에 크로스 체크 섹션 추가

피드백 반영 후 WORK-SPEC.html 마지막에 다음 섹션을 추가한다:
...
WORK-SPEC.html 생성 후, 작업 추적을 위한 Track 디렉토리를 생성한다.
...
WORK-SPEC.html의 내용을 Track 디렉토리에 번호 접두사를 붙여 복사한다.
(`.claude/docs/WORK-SPEC.html` → `.claude/tracks/{track_id}/*_WORK-SPEC.html`)
...
> Phase 구조는 WORK-SPEC.html의 "작업 단계 (Phase)" 섹션을 기반으로 생성한다.
...
```json
{
  ...
  "work_spec_path": "{WORK-SPEC.html 경로}",
  "total_phases": {N},
  "current_phase": 0
}
```

### WORK-SPEC.html에 Track 참조 추가

WORK-SPEC.html 상단 메타데이터에 Track ID를 추가한다:
...
WORK-SPEC.html 및 Track 생성 후 **EnterPlanMode**로 전환하여 구현 계획을 제안한다.

사용자가 WORK-SPEC.html 내용을 검토하고 승인하면 `/work-plan-start`로 실행할 수 있다.
...
- **WORK-SPEC.html 위치**: req.md와 같은 디렉토리에 생성한다 (기본: `.claude/docs/`).
- **수정 가능**: 사용자가 WORK-SPEC.html를 직접 수정한 후 `/work-plan-start`를 실행할 수 있다.
- **크로스 체크 실패 시**: 외부 AI 크로스 체크가 실패해도 Claude 단독으로 생성한 WORK-SPEC.html는 유효하다.
