---
description: "Work Plan [path] [--no-cross-check] - req.md → WORK-SPEC.md 작업 명세서 생성"
...
/work-plan --no-cross-check            # 크로스 체크 건너뛰기
/work-plan path/to/req.md --no-cross-check  # 경로 + 크로스 체크 건너뛰기
```

## 0단계: 인자 파싱

`$ARGUMENTS`에서 옵션과 경로를 분리한다.

| 옵션 | 설명 |
|------|------|
| `--no-cross-check` | 4단계(Gemini/Codex 크로스 체크)를 건너뛴다 |

- `--no-cross-check`를 제거한 나머지 텍스트가 req.md 경로가 된다
- 옵션이 없으면 기본 동작(크로스 체크 포함)으로 진행

## 1단계: req.md 읽기

인자 파싱 후 경로가 있으면 해당 경로, 없으면 `.claude/docs/req.md`를 읽는다.

```
경로 우선순위:
1. $ARGUMENTS에서 옵션을 제외한 경로
...
> `--no-cross-check` 옵션이 지정된 경우 이 단계를 **전체 건너뛰고** 5단계로 이동한다.
> 완료 보고의 크로스 체크 섹션에 "⏭️ 건너뜀 (--no-cross-check)" 표시.
