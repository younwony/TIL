---
description: PR 생성 전 현재 브랜치의 변경사항을 4명의 전문 리뷰어 에이전트 팀 + Gemini/Codex 크로스 리뷰로 자체 리뷰하고 SELF-REVIEW.html를 생성합니다.
effort: high
...
PR 생성 전에 현재 브랜치의 변경사항을 4명의 전문 리뷰어 에이전트 + Gemini/Codex 크로스 리뷰를 **병렬로** 실행하여 자체 리뷰하고, 결과를 `SELF-REVIEW.html` 문서로 생성한다.

> **AI 크로스 체크(Gemini/Codex)는 `gemini-check` / `codex-check` 하네스로 위임한다.**
> 본 커맨드는 더 이상 `gemini -p`, `codex exec -`, `codex review --base`를 직접 호출하지 않는다.
...
> 4명의 리뷰 에이전트(performance/security/test-coverage/convention)는 변경 없음. 그대로 Task 도구로 호출한다.
...
## 4단계: 결과 통합 → SELF-REVIEW.html 생성

6명의 리뷰 결과를 통합하여 `{DOC_DIR}/*_SELF-REVIEW.html` 파일을 생성한다 (활성 Track이 있으면 Track 디렉토리, 없으면 프로젝트 루트에 `SELF-REVIEW.html`):

산출 문서는 `html-doc` 스킬의 규칙을 따라 자체 완결 HTML로 작성한다.
`~/.claude/skills/productivity/html-doc/references/template.html`을 skeleton으로 사용하고,
리뷰어별 지적사항은 `<details>` collapsible과 상태 배지(badge-ok/badge-warn/badge-err)로,
심각도 분포는 비교표로 표현한다. 다이어그램이 필요하면 인라인 `<svg>`로 임베드한다.

HTML 구조는 다음 섹션 구성으로 작성한다:

- **`<header>`**: 브랜치명, 리뷰 일시, 리뷰 방식 (Agent Team 4명 + Cross Review)
- **변경 개요 `<section>`**: 커밋 수·변경 파일 수·추가/삭제 라인, 커밋 히스토리 `<table>`, 변경 파일 `<table>`
- **종합 리뷰 요약 `<section>`**: 관점별 상태 배지(badge-ok/badge-warn/badge-err)를 포함한 `<table>`, 심각도 집계 비교 `<table>`
