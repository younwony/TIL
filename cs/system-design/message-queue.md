...
> `[4] 심화` · 선수 지식: [확장성](./scalability.md), 비동기 프로그래밍
...
- **면접**: "Kafka vs RabbitMQ", "메시지 순서 보장" 등 분산 시스템 설계 필수 질문
- **기반 지식**: 이벤트 드리븐 아키텍처, 비동기 처리의 핵심 기술
...
**메시지 큐**를 음식점의 주문 시스템에 비유할 수 있습니다.

### 메시지 큐 = 주문서 꽂이

**메시지 큐 없이 (동기 통신)**
손님이 직접 주방에 가서 요리사에게 주문하고, 요리가 완성될 때까지 기다립니다. 요리사가 바쁘면 손님이 계속 기다려야 하고, 요리사가 쉬고 있으면 손님이 없어서 놀게 됩니다.

**메시지 큐 있을 때 (비동기 통신)**
손님이 카운터에서 주문하면, 주문서가 꽂이(Queue)에 순서대로 꽂힙니다. 요리사는 자기 속도에 맞춰 주문서를 하나씩 뽑아서 처리합니다.

| 비유 | 메시지 큐 개념 |
|------|---------------|
| 손님 | Producer (메시지 생산자) |
| 주문서 꽂이 | Message Queue |
| 주문서 | Message |
| 요리사 | Consumer (메시지 소비자) |

### 왜 필요한가?

- **바쁜 시간 대응**: 점심시간에 주문이 몰려도 주문서가 쌓이면서 버퍼 역할
- **요리사 추가**: 바쁘면 요리사(Consumer)만 추가하면 됨
- **손님 대기 불필요**: 주문 후 번호표 받고 자리에서 기다리면 됨

### RabbitMQ vs Kafka = 우체국 vs 신문사

| 비유 | RabbitMQ | Kafka |
|------|----------|-------|
| 역할 | 우체국 (메시지 브로커) | 신문사 (이벤트 스트리밍) |
| 메시지 처리 | 편지 배달 후 삭제 | 신문 보관 (다시 볼 수 있음) |
| 구독 방식 | 받는 사람 지정 | 신문 구독 (여러 명이 같은 신문 읽기) |
| 적합한 상황 | 1:1 작업 전달 | 이벤트 로그, 여러 서비스가 같은 이벤트 필요 |

## 메시지 큐 아키텍처

![메시지 큐 기본 구조](./images/mq-basic.svg)
...
![RabbitMQ 구조](./images/rabbitmq-flow.svg)
...
![Kafka 아키텍처](./images/kafka-architecture.svg)
...
- **Q: Kafka와 RabbitMQ의 차이점은 무엇인가요?**
  - A: Kafka는 분산 로그 기반 스트리밍 플랫폼으로 대용량 처리와 메시지 재처리에 강점이 있고, RabbitMQ는 AMQP 기반 메시지 브로커로 복잡한 라우팅과 메시지 단위 ACK에 강점이 있습니다. Kafka는 Consumer가 Pull 방식으로 메시지를 가져가고, RabbitMQ는 Push 방식입니다. **왜 이렇게 답해야 하나요?** Pull 방식은 Consumer가 처리 속도를 조절할 수 있어 백프레셔 문제가 없고, 배치 처리가 가능합니다. Push 방식은 실시간성이 좋지만 느린 Consumer가 과부하를 받을 수 있습니다. 또한 Kafka의 로그 기반 저장은 이벤트 소싱과 재처리에 유리합니다.

- **Q: Consumer Group은 무엇이고 왜 필요한가요?**
  - A: Consumer Group은 Kafka에서 같은 토픽을 구독하는 Consumer들의 논리적 그룹입니다. 그룹 내 Consumer들은 파티션을 분담하여 병렬 처리하고, 그룹 간에는 같은 메시지를 독립적으로 소비합니다. 이를 통해 확장성과 내결함성을 제공합니다. **왜 이렇게 답해야 하나요?** Consumer Group이 없으면 모든 Consumer가 모든 메시지를 받아 중복 처리하거나, 메시지를 나눠받을 방법이 없어 확장이 불가능합니다. 그룹별로 독립적인 Offset을 관리하여 주문 서비스와 알림 서비스가 같은 이벤트를 각자 소비할 수 있습니다.

- **Q: 메시지 큐에서 메시지 순서를 보장하려면 어떻게 해야 하나요?**
  - A: Kafka에서는 같은 파티션 키를 사용하여 관련 메시지를 같은 파티션으로 보내면 순서가 보장됩니다. RabbitMQ에서는 단일 큐와 단일 Consumer를 사용하거나, Message Sequencing 패턴을 적용합니다. 단, 순서 보장은 처리량과 트레이드오프가 있습니다. **왜 이렇게 답해야 하나요?** 순서 보장을 위해 병렬 처리를 포기하면 처리량이 크게 감소합니다. 사용자별 주문 이벤트처럼 부분 순서만 필요한 경우 userId를 파티션 키로 사용하면, 사용자별 순서는 보장하면서도 전체적으로는 병렬 처리가 가능합니다.

## 연관 문서

| 문서 | 연관성 | 난이도 |
|------|--------|--------|
| [확장성](./scalability.md) | 비동기 처리로 확장성 확보 | 입문 |
| [일관성](./consistency.md) | 최종 일관성 모델 | 중급 |
| [서킷브레이커](./circuit-breaker.md) | 메시지 큐를 통한 장애 격리 | 심화 |
| [선착순 쿠폰 시스템](./flash-sale-system.md) | Kafka 비동기 처리 활용 예시 | 심화 |
| [대규모 시스템 설계](./large-scale-system.md) | 메시지 큐를 포함한 전체 아키텍처 | 심화 |
