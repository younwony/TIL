...
├────────────────────────────────────────────────────────────────┤
│                                                                 │
│   Layer 7 (Application)  ←── L7 로드밸런서                      │
│   ┌─────────────────────────────────────────────────────────┐  │
│   │ HTTP 헤더, URL, 쿠키 기반 라우팅                         │  │
│   │ 예: /api/* → API 서버, /static/* → 정적 서버            │  │
│   └─────────────────────────────────────────────────────────┘  │
│                          │                                      │
│   Layer 4 (Transport)    ←── L4 로드밸런서                      │
│   ┌─────────────────────────────────────────────────────────┐  │
│   │ IP, Port 기반 라우팅                                     │  │
│   │ 패킷 내용을 보지 않음 (더 빠름)                          │  │
│   └─────────────────────────────────────────────────────────┘  │
...
│                                                                 │
│   요청 1: 로그인 → 서버A (세션 저장)                            │
│   요청 2: 장바구니 → 서버B (세션 없음!) ← 문제 발생             │
│                                                                 │
│   ──────────────────────────────────────────────────────────    │
│                                                                 │
│   [해결 방법 1: Sticky Session]                                 │
│                                                                 │
│   쿠키에 서버 ID 저장 → 같은 서버로 라우팅                      │
│   단점: 서버 장애 시 세션 유실                                  │
│                                                                 │
│   ──────────────────────────────────────────────────────────    │
│                                                                 │
