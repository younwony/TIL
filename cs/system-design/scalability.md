...
- **면접**: "수평 확장과 수직 확장 차이" 등 시스템 설계 면접의 가장 기초적인 질문
- **기반 지식**: 로드밸런싱, 캐싱, 샤딩 등 모든 고급 기술의 전제 조건
...
![수직 확장 vs 수평 확장](./images/scalability-comparison.svg)
...
![로드 밸런서 구조](./images/load-balancer.svg)
...
- Q: 수직 확장과 수평 확장의 차이점과 각각의 장단점은?
  - A: 수직 확장은 단일 서버 성능 향상으로 구현이 단순하지만 한계가 있고, 수평 확장은 서버 추가로 이론상 무한 확장 가능하지만 분산 시스템 복잡도가 증가한다. 초기에는 수직 확장, 규모가 커지면 수평 확장이 적합하다. **왜 이렇게 답해야 하나요?** 수직 확장은 하드웨어 한계가 있고 비용이 지수적으로 증가합니다(4코어→32코어가 8배 비용이 아닌 20배). 수평 확장은 서버를 추가하면 선형적으로 비용이 증가하지만, 네트워크 지연, 데이터 일관성, 분산 트랜잭션 등 복잡도가 증가합니다.

- Q: 수평 확장 시 세션 관리는 어떻게 하는가?
  - A: Sticky Session(로드밸런서가 같은 서버로 라우팅), Session Clustering(서버 간 세션 공유), 외부 저장소(Redis) 사용 방식이 있다. 외부 저장소 방식이 가장 확장성이 좋고 권장된다. **왜 이렇게 답해야 하나요?** Sticky Session은 서버 장애 시 세션이 유실되고 로드밸런싱이 불균형해집니다. Session Clustering은 서버 간 동기화 오버헤드가 크고 네트워크 파티션에 취약합니다. Redis는 중앙화된 세션 저장소로 서버가 무상태가 되어 자유롭게 확장 가능합니다.

- Q: 데이터베이스 확장 전략에는 어떤 것들이 있는가?
  - A: 읽기 확장은 레플리케이션(Master-Slave), 쓰기 확장은 샤딩을 사용한다. 캐시 레이어 추가, 쿼리 최적화, 커넥션 풀링도 함께 적용한다. 샤딩은 조인 어려움, 리밸런싱 복잡도 등의 트레이드오프가 있다. **왜 이렇게 답해야 하나요?** 레플리케이션은 읽기를 분산하지만 쓰기는 여전히 단일 Master의 병목입니다. 샤딩은 쓰기를 분산하지만 샤드 간 조인이 불가능하여 애플리케이션 레벨에서 처리해야 하고, 데이터 재분배(리샤딩)가 매우 복잡합니다. 따라서 캐싱으로 DB 부하를 먼저 줄이고, 읽기 레플리카로 읽기를 분산한 후, 마지막으로 샤딩을 고려해야 합니다.

## 연관 문서

| 문서 | 연관성 | 난이도 |
|------|--------|--------|
| [가용성](./availability.md) | 수평 확장을 통한 고가용성 | 입문 |
| [캐싱](./caching.md) | DB 부하 분산 전략 | 중급 |
| [메시지 큐](./message-queue.md) | 비동기 처리로 확장성 확보 | 심화 |
| [대규모 시스템 설계](./large-scale-system.md) | 확장성을 고려한 전체 시스템 설계 | 심화 |
