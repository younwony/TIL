...
> `[3] 중급` · 선수 지식: Linux 기본 명령어, 가상화 개념
...
- **면접**: "Docker와 VM의 차이", "멀티스테이지 빌드" 등 실무 경험 확인 질문
- **기반 지식**: Kubernetes, CI/CD 파이프라인 이해의 전제 조건
...
![컨테이너 vs 가상머신](./images/docker-vs-vm.svg)
...
![Docker 아키텍처](./images/docker-architecture.svg)
...
- **Q: 컨테이너와 가상머신의 차이점은 무엇인가요?**
  - A: 가상머신은 Hypervisor 위에 각각의 Guest OS를 실행하여 완전한 격리를 제공하지만 무겁습니다. 컨테이너는 Host OS의 커널을 공유하고 프로세스 수준의 격리를 제공하여 가볍고 빠릅니다. 컨테이너는 초 단위로 시작되고 MB 단위 크기인 반면, VM은 분 단위 시작과 GB 단위 크기입니다. **왜 이렇게 답해야 하나요?** 커널을 공유하므로 네임스페이스와 cgroup으로 격리하지만 같은 커널을 쓰기에 가볍습니다. 하지만 커널 취약점은 호스트까지 영향을 줄 수 있어 완전한 격리가 필요하면 VM을 사용해야 합니다.

- **Q: Docker 이미지와 컨테이너의 차이는 무엇인가요?**
  - A: 이미지는 애플리케이션을 실행하기 위한 파일 시스템과 설정을 포함하는 읽기 전용 템플릿입니다. 컨테이너는 이미지를 기반으로 생성된 실행 가능한 인스턴스로, 이미지 위에 쓰기 가능한 레이어가 추가됩니다. 클래스와 객체의 관계와 유사합니다. **왜 이렇게 답해야 하나요?** Copy-on-Write 방식으로 여러 컨테이너가 같은 이미지 레이어를 공유하므로 메모리와 디스크를 절약합니다. 컨테이너가 변경한 내용은 별도 레이어에 기록되어 이미지는 불변입니다.

- **Q: 멀티스테이지 빌드란 무엇이고 왜 사용하나요?**
  - A: 멀티스테이지 빌드는 하나의 Dockerfile에서 여러 FROM 명령을 사용하여 빌드 단계와 실행 단계를 분리하는 기법입니다. 빌드에 필요한 도구(컴파일러, 빌드 도구 등)를 최종 이미지에서 제외하여 이미지 크기를 줄이고 보안을 강화할 수 있습니다. **왜 이렇게 답해야 하나요?** Node.js 빌드 시 node_modules, 소스 파일 등으로 수백 MB가 되지만, 실행에는 빌드된 결과물만 필요합니다. 멀티스테이지로 분리하면 이미지 크기를 1GB에서 100MB로 줄일 수 있고, 공격 표면도 감소시킵니다.

## 연관 문서

| 문서 | 연관성 | 난이도 |
|------|--------|--------|
| [Kubernetes](./kubernetes.md) | 컨테이너 오케스트레이션 플랫폼 | 심화 |
| [서버리스 컴퓨팅](./serverless.md) | 컨테이너 기반 서버리스 (AWS Fargate) | 중급 |
| [확장성](./scalability.md) | 컨테이너 기반 수평 확장 | 입문 |
