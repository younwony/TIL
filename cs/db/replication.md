...
│      │ ←───── ACK ─────────────────┘                  │     │
│      ▼                                                       │
│   OK (모든 Replica 완료 후 응답)                             │
│                                                              │
│   장점: 데이터 일관성 보장                                    │
│   단점: 느림, Replica 장애 시 쓰기 불가                       │
│                                                              │
└─────────────────────────────────────────────────────────────┘
...
│                                                              │
│   장점: 빠름, 고가용성                                        │
│   단점: 복제 지연(Replication Lag), 데이터 손실 가능         │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
─────────────────────────────────────────────────────→
    T1         T2         T3         T4
    │          │          │          │
 Primary:     INSERT     SELECT     (데이터 있음)
    │          │          │          │
 Replica:     (아직 없음) (아직 없음) INSERT

T2 시점에 Replica에서 읽으면 → 데이터 없음!
...
2. Monotonic Reads
   - 같은 세션은 같은 Replica에서 읽기

3. Bounded Staleness
   - 최대 지연 시간 제한, 초과 시 Primary에서 읽기

4. CQRS
   - Command(쓰기)와 Query(읽기) 분리
```
...
Failover 유형:
1. 수동 Failover: 관리자가 직접 승격
2. 자동 Failover: 감시 시스템이 자동 승격 (Sentinel, Orchestrator)
```
