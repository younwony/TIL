...
> 데이터를 체계적으로 저장, 관리, 조회할 수 있도록 구조화된 저장소

`#데이터베이스` `#Database` `#DB` `#DBMS` `#RDBMS` `#NoSQL` `#SQL` `#데이터저장소` `#데이터관리` `#영속성` `#데이터무결성` `#데이터일관성` `#동시성제어` `#트랜잭션` `#인덱스` `#스키마` `#테이블` `#문서` `#키-값` `#컬럼` `#그래프` `#MySQL` `#PostgreSQL` `#MongoDB` `#Redis` `#데이터베이스설계` `#데이터모델링` `#데이터아키텍처`

## 왜 알아야 하는가?

- **실무**: 모든 애플리케이션의 데이터 영속성을 위해 데이터베이스를 사용합니다. CRUD 기능, 사용자 정보, 주문 내역 등 대부분의 비즈니스 데이터는 데이터베이스에 저장됩니다.
- **면접**: "RDBMS와 NoSQL의 차이점은 무엇인가요?", "어떤 상황에서 어떤 데이터베이스를 선택하나요?" 등 기본 개념에 대한 질문이 반드시 나옵니다.
- **기반 지식**: 인덱스, 트랜잭션, 정규화 등 모든 데이터베이스 심화 주제의 기반이 되는 개념입니다.

## 좋은 시스템 설계 요소

![좋은 시스템 설계 요소](./images/system-design-elements.svg)
...
**데이터베이스**을 도서관에 비유할 수 있습니다.

도서관에는 수많은 책(데이터)이 있지만, 그냥 쌓아두면 찾기가 어렵습니다. 그래서:
- **분류 번호**(스키마)로 책을 정리합니다
- **목록 카드**(인덱스)로 원하는 책을 빠르게 찾습니다  
- **대출 규칙**(트랜잭션)으로 책의 상태를 관리합니다
- **사서**(DBMS)가 전체를 관리하고 규칙을 지킵니다

예를 들어, "컴퓨터 공학" 책을 찾을 때:
- 분류 번호(004)로 해당 분야 서가로 이동
- 목록 카드에서 책 제목으로 정확한 위치 확인
- 대출 가능한 상태인지 확인 후 대출 처리

이것이 데이터베이스의 핵심입니다: **체계적인 정리와 빠른 검색, 일관된 관리**.
...
#### 기본 아키텍처 다이어그램
![DBMS 아키텍처](./images/dbms-architecture.svg)

#### 전문가용 상세 아키텍처 (Mermaid)
![DBMS 전문가용 아키텍처](./images/dbms-architecture-professional.svg)

#### C4 모델 컨테이너 뷰
![DBMS C4 아키텍처](./images/dbms-c4-architecture.svg)

DBMS(Database Management System)는 데이터베이스를 관리하는 핵심 소프트웨어로, 다음과 같은 주요 구성요소로 이루어져 있습니다.

#### 1. 질의 처리기 (Query Processor)

- **SQL 해석**: 사용자가 작성한 SQL 쿼리를 파싱하고 분석
- **최적화**: 쿼리 실행 계획을 수립하고 비용 최적화
- **실행**: 최적화된 계획에 따라 실제 쿼리 실행

#### 2. 트랜잭션 관리자 (Transaction Manager)

- **ACID 보장**: 원자성(Atomicity), 일관성(Consistency), 독립성(Isolation), 지속성(Durability)
- **동시성 제어**: 여러 사용자의 동시 접근 관리
- **회복 관리**: 장애 발생 시 데이터 복구

#### 3. 저장소 관리자 (Storage Manager)

- **디스크 I/O**: 물리적 저장소와의 데이터 입출력 관리
- **버퍼 관리**: 메모리와 디스크 간의 데이터 효율적 이동
- **인덱스 관리**: 데이터 검색 속도 향상을 위한 인덱스 구조 관리
