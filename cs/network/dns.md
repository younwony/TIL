...
- **성능 영향**: DNS 조회 시간이 전체 페이지 로딩 시간에 직접적인 영향
- **장애 대응**: DNS 장애 시 서비스 전체가 중단되므로 이해 필수
- **실무 활용**: 도메인 설정, TTL 관리, 서버 마이그레이션 시 필수 지식
- **보안 중요성**: DNS Spoofing, DDoS 등 보안 위협에 대한 이해와 대응 필요
...
- [HTTP/HTTPS](./http-https.md) - DNS 조회 후 HTTP 통신이 시작됨
- [TCP/IP](./tcp-ip.md) - DNS는 주로 UDP를 사용하며 TCP/IP 계층 위에서 동작
- [CDN](./cdn.md) - GeoDNS를 활용한 CDN 엣지 서버 라우팅
- [Load Balancing](./load-balancing.md) - DNS 라운드 로빈을 통한 부하 분산
