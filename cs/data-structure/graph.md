...
> `[4] 심화` · 선수 지식: [트리 (Tree)](./tree.md), [스택과 큐 (Stack & Queue)](./stack-queue.md), [해시 테이블 (Hash Table)](./hash-table.md)
...
- **면접**: DFS/BFS, 최단 경로, 사이클 탐지, 위상 정렬 등은 코딩 면접의 핵심 알고리즘입니다. 그래프 문제는 난이도가 높아 면접에서 실력을 평가하는 좋은 수단입니다.
- **기반 지식**: 알고리즘 설계의 정점으로, 다양한 자료구조와 알고리즘 기법을 종합적으로 활용하는 능력을 기릅니다. 실무 문제를 추상화하여 그래프로 모델링하는 능력이 중요합니다.
...
**비연결 그래프 (Disconnected Graph)**: A, B, C는 연결되어 있지만, D, E와는 분리되어 있습니다.

#### 6. 순환 그래프 (Cyclic Graph) vs 비순환 그래프 (Acyclic Graph)

**순환 (Cycle)**: 시작 정점으로 돌아오는 경로 존재 (예: A → B → C → D → A)
...
![인접 행렬](./images/graph-adj-matrix.svg)
...
![인접 리스트](./images/graph-adj-list.svg)
...
- [트리 (Tree)](./tree.md) - 그래프의 특수한 형태(사이클 없는 연결 그래프)
- [스택과 큐 (Stack & Queue)](./stack-queue.md) - DFS(스택), BFS(큐)에서 사용
- [해시 테이블 (Hash Table)](./hash-table.md) - 인접 리스트 구현과 방문 추적에 활용
- [힙 (Heap)](./heap.md) - 다익스트라, 프림 알고리즘에서 우선순위 큐로 사용
