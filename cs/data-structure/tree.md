...
> `[3] 중급` · 선수 지식: [연결 리스트 (Linked List)](./linked-list.md), [스택과 큐 (Stack & Queue)](./stack-queue.md)
...
- **면접**: 트리 순회, BST 조작, 트리 재구성 등은 코딩 면접의 핵심 주제입니다. 재귀적 사고와 포인터 조작 능력을 평가하는 좋은 문제들이 많습니다.
- **기반 지식**: 힙, 세그먼트 트리, 트라이 등 고급 자료구조의 기반이며, 그래프 이론과 알고리즘 설계의 중요한 토대입니다.
...
![조직도 트리 구조](./images/tree-organization.svg)
...
![트리 용어 설명](./images/tree-terminology.svg)
...
![이진 트리 종류](./images/tree-binary-types.svg)
...
![트리 순회 예시](./images/tree-traversal.svg)
...
- [연결 리스트 (Linked List)](./linked-list.md) - 트리와 유사한 노드 기반 구조
- [스택과 큐 (Stack & Queue)](./stack-queue.md) - DFS(스택), BFS(큐)를 사용하는 트리 순회
- [힙 (Heap)](./heap.md) - 완전 이진 트리 기반의 우선순위 큐
- [그래프 (Graph)](./graph.md) - 트리는 사이클이 없는 특수한 그래프
- [해시 테이블 (Hash Table)](./hash-table.md) - 트리와 비교되는 검색 자료구조
