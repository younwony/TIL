...
- **면접**: 배열의 시간 복잡도, 동적 배열의 동작 원리, 캐시 친화성 등은 기본 면접 질문입니다. 투 포인터, 슬라이딩 윈도우 등 배열 기반 알고리즘도 자주 출제됩니다.
- **기반 지식**: 스택, 큐, 힙 등 다른 자료구조의 구현 기반이 되며, 메모리 구조와 성능 최적화를 이해하는 첫 걸음입니다.
...
- [연결 리스트 (Linked List)](./linked-list.md) - 배열과 비교되는 동적 자료구조
- [스택과 큐 (Stack & Queue)](./stack-queue.md) - 배열로 구현 가능한 자료구조
- [해시 테이블 (Hash Table)](./hash-table.md) - 배열 기반의 키-값 저장 구조
