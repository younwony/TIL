...
> `[4] 심화` · 선수 지식: [OOP](./oop.md), [클린 코드](./clean-code.md)
...
- **동시성 안전**: 불변 데이터와 순수 함수로 멀티스레드 문제 해결
- **코드 간결성**: Stream API로 복잡한 데이터 처리를 선언적으로 표현
- **면접 빈출**: Stream, Optional, 람다 표현식은 실무 필수이자 면접 단골 주제
- **최신 트렌드**: React, Redux 등 프론트엔드 프레임워크도 함수형 개념 적용
...
**함수형 프로그래밍**을 요리 레시피에 비유할 수 있습니다.

- **명령형 프로그래밍** = 요리사가 직접 재료를 손질하며 요리
  - "냄비에 물을 넣고, 불을 켜고, 끓으면 면을 넣고..."
  - 요리사의 상태(피로도, 기분)가 결과에 영향
  - 같은 레시피로도 매번 다른 맛이 날 수 있음

- **함수형 프로그래밍** = 자동화된 요리 기계
  - "재료 A + 재료 B → 결과 C" (순수 함수)
  - 같은 입력에 항상 같은 출력
  - 기계 외부 상태를 건드리지 않음 (부수 효과 없음)
  - 여러 기계를 연결해서 복잡한 요리 완성 (함수 합성)

**핵심 비유:**
- **순수 함수** = 계산기 (같은 버튼 → 같은 결과)
- **불변 데이터** = 사진 (원본은 그대로, 수정 시 새 사진 생성)
- **고차 함수** = 기계를 만드는 기계
...
### 필수 선행 학습
- [OOP](./oop.md) - 객체지향 개념을 알아야 함수형과의 차이 이해 가능
- [클린 코드](./clean-code.md) - 순수 함수, 부수 효과 개념의 기초

### 관련 심화 주제
- [디자인 패턴](./design-pattern.md) - Strategy 패턴을 고차 함수로 대체
- [API 설계](./api-design.md) - 불변 객체 기반 API 설계

### 실무 적용
- [동시성 프로그래밍](./concurrency.md) - 불변성을 활용한 안전한 멀티스레드
- [테스트](./test.md) - 순수 함수는 테스트하기 쉬움
