...
**클린 코드**를 도서관 책 정리에 비유할 수 있습니다.

- **더러운 코드** = 책이 무작위로 쌓여있는 도서관
  - 원하는 책을 찾으려면 모든 책을 뒤져야 함
  - 새 책을 넣을 자리를 찾기 어려움
  - 책이 많아질수록 찾기 더 어려워짐

- **클린 코드** = 체계적으로 정리된 도서관
  - 분류 번호(네이밍)로 바로 찾을 수 있음
  - 각 선반(함수)은 한 가지 주제만 담당
  - 목차(구조)가 명확해서 전체 파악이 쉬움
  - 새 책도 규칙에 따라 쉽게 배치

**왜 중요한가?**
코드는 작성하는 시간보다 읽는 시간이 10배 이상입니다. 6개월 후 내 코드를 다시 볼 때, 깔끔하게 정리되어 있으면 금방 이해하고 수정할 수 있습니다. 그렇지 않으면 처음부터 다시 분석해야 합니다.
