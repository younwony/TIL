...
- **개발자 간 공통 언어**: "여기 Strategy 패턴을 쓰자" → 즉시 의도 전달
- **코드 품질 향상**: SOLID 원칙을 자연스럽게 따르게 됨
- **면접 필수**: 실무 경험과 설계 능력을 평가하는 단골 질문
- **리팩토링 방향**: 기존 코드를 개선할 때 어떤 패턴을 적용할지 판단 가능
...
### 필수 선행 학습
- [OOP](./oop.md) - 객체지향 원칙(SOLID)을 알아야 패턴 이해 가능
- [클린 코드](./clean-code.md) - 패턴을 적용하기 전에 클린 코드 원칙 숙지

### 관련 심화 주제
- [함수형 프로그래밍](./functional-programming.md) - Strategy 패턴을 고차 함수로 대체 가능
- [API 설계](./api-design.md) - Facade, Adapter 패턴을 API 설계에 적용

### 실무 적용
- [리팩토링](./refactoring.md) - 코드 스멜을 디자인 패턴으로 개선
- [테스트](./test.md) - 의존성 주입과 Mock 객체를 활용한 테스트
