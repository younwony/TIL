...
> `[3] 중급` · 선수 지식: [Spring DI/IoC](./spring-di-ioc.md), [HTTP](../network/http.md)
...
│    │                                                         │
│    │ 1) Handler Mapping                                      │
│    │    어떤 Controller의 어떤 메서드?                       │
│    ▼                                                         │
│  ┌────────────────────────────────────────┐                 │
│  │ Handler Adapter                        │                 │
│  │ - 파라미터 바인딩 (@RequestBody 등)    │                 │
│  │ - Controller 메서드 호출               │                 │
│  └────────────────────────────────────────┘                 │
│    │                                                         │
│    ▼                                                         │
...
│  │ - 비즈니스 로직 수행                   │                 │
│  │ - Model 데이터 설정                    │                 │
│  │ - View 이름 반환 또는 @ResponseBody    │                 │
│  └────────────────────────────────────────┘                 │
│    │                                                         │
│    │ 2) ViewResolver (뷰가 있는 경우)                        │
│    │    뷰 이름 → 실제 View 객체                             │
│    │                                                         │
│    │ 3) HttpMessageConverter (@ResponseBody)                │
│    │    객체 → JSON/XML                                      │
│    ▼                                                         │
│  Client                                                      │
│    HTTP Response                                             │
...
│    ▼                                                         │
│  ┌──────────────────┐                                       │
│  │ Interceptor      │ ← Spring 스펙, Handler 정보 접근      │
│  │ (인증, 로깅)     │   특정 경로에 선택적 적용              │
│  └──────────────────┘                                       │
│    │                                                         │
│    ▼                                                         │
...
│  └──────────────────┘                                       │
│                                                              │
│  Filter: 웹 컨테이너 수준, Spring Bean 아님 (주입 어려움)    │
│  Interceptor: Spring 수준, Bean 주입 가능                   │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
