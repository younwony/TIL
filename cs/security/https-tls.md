...
- **면접**: "HTTPS는 어떻게 안전한가요", "TLS 핸드셰이크 과정", "왜 대칭키와 비대칭키를 함께 사용하나요"는 네트워크/보안 면접의 핵심 질문입니다. 웹 개발자라면 반드시 설명할 수 있어야 합니다.
- **기반 지식**: 인증서, CA, PKI 등 보안 인프라의 작동 원리를 이해하는 기반이 됩니다. 또한 REST API, OAuth, WebSocket 등 모든 웹 통신 기술의 보안 계층을 이해할 수 있습니다.
...
| 문서 | 연관성 | 난이도 |
|------|--------|--------|
| [암호화](./cryptography.md) | TLS는 대칭키/비대칭키 암호화를 결합하여 구현 (선수 지식) | Intermediate |
| [인증과 인가](./authentication-authorization.md) | 인증서를 통한 서버 인증, HTTPS 위에서 세션/토큰 전송 (선수 지식) | Beginner |
| [웹 보안](./web-security.md) | HTTPS는 중간자 공격, 도청 등 네트워크 계층 공격 방어 | Intermediate |
| [OAuth 2.0과 JWT](./oauth-jwt.md) | OAuth 토큰은 HTTPS를 통해 안전하게 전송 | Advanced |
