...
### 주요 하이라이트
- **세션 컨텍스트 Recap 기능 추가**: `/config` 또는 `/recap` 명령으로 세션 요약 제공
- **내장 슬래시 커맨드를 Skill 도구에서 발견 가능**: 스킬 시스템과 기본 커맨드 통합
- **프롬프트 캐시 TTL 환경변수 지원**: 여러 제공자(Provider)별로 TTL 설정 가능

### 새 기능
- 여러 제공자(Provider)에 걸친 프롬프트 캐시 TTL 설정 환경변수 추가
- 세션 컨텍스트 Recap 기능 (`/config` 또는 `/recap`으로 제어)
- Skill 도구에서 내장 슬래시 커맨드 검색 가능
- `/undo`를 `/rewind`의 별칭으로 추가
- 상세 트랜스크립트 뷰에 verbose 표시기 추가

### 개선
- `/model` 전환 시 대화 중간 경고 표시 개선
- `/resume` 선택기에 현재 디렉토리 기본값 적용
- 레이트 리밋 및 서버 에러 메시지 다듬기
- 문법 파일 온디맨드 로딩으로 파일 작업 메모리 사용량 감소
- 사전조건 실패(Precondition Failure) 에러 가시성 향상

### 버그 수정
- 로그인 코드 입력창의 붙여넣기 기능 수정
- 텔레메트리 비활성 구독자의 프롬프트 캐시 TTL 처리 수정
- 자동 모드에서 Agent 도구 권한 확인 동작 수정
- 환경 파일이 주석으로 끝날 때 Bash 도구 출력 수정
- `--resume` 시 세션 이름 및 커스텀 설정 유지 수정
- `--teleport` 이후 터미널 이스케이프 코드 처리 수정
- `/feedback` 재시도 기능 수정
- Remote Control 세션 제목 덮어쓰기 문제 수정
- 대용량 세션의 트랜스크립트 처리 수정
- 트랜스크립트 쓰기 실패 로깅 수정
- 응답의 다이어크리틱 마크(발음기호) 처리 수정
- 프로젝트 간 플러그인 자동 업데이트 동작 수정
...
### 주요 하이라이트
- 장시간 작업 시 "thinking" 힌트를 더 일찍 표시하도록 개선

### 개선
- 확장 실행(extended operations) 중 thinking 힌트 조기 노출
...
### 주요 하이라이트
- **`EnterWorktree` 도구에 `path` 파라미터 추가**: 기존 worktree 간 전환 지원
- **PreCompact 훅 지원 + 차단 기능**: 컴팩션 전 훅으로 동작 제어 가능
- **플러그인 백그라운드 모니터 지원**: 매니페스트 설정으로 백그라운드 작업 관리

### 새 기능
- `EnterWorktree` 도구의 `path` 파라미터 (기존 worktree 전환)
- PreCompact 훅 지원 (blocking 가능)
- 플러그인 매니페스트를 통한 백그라운드 모니터 지원
- `/proactive`를 `/loop`의 별칭으로 추가

### 개선
- API 스트림 핸들링에 타임아웃 및 재시도 로직 추가
- 네트워크 에러 메시지에 즉시 재시도 프롬프트 표시
- 대용량 단일 라인 콘텐츠는 파일 쓰기 표시 시 잘라서 보여줌
- `/doctor`에 상태 아이콘 및 자동 수정(Fix) 기능 추가
- `/config` 라벨과 설명 개선
- 스킬 description 길이 제한 확장 + 초과 시 경고
- `WebFetch`에서 style/script 태그 제거로 토큰 효율 향상
- squash-merge된 PR에 대한 stale agent worktree 정리 개선
- MCP 출력 truncation을 포맷별 가이드로 정교화

### 버그 수정
- 대기 중 메시지의 이미지 첨부 처리 수정
- 긴 대화에서의 화면 렌더링 수정
- 멀티라인 선택 및 응답의 공백 처리 수정
- 클릭 가능한 파일 링크 있는 bash 출력 깨짐 수정
- 여러 터미널 모드의 줄바꿈 삽입 수정
- 중복 도구 표시 텍스트 수정
- focus 모드에서 큐에 쌓인 프롬프트 가시성 수정
- 예약 작업(scheduled task) 재트리거 수정
- 팀/엔터프라이즈 사용자의 알림 처리 수정
- 플러그인 의존성 설치 수정
- 마켓플레이스 플러그인 자동 업데이트 수정
- resume 세션 힌트 수정
- 피드백 설문 키바인딩 수정
- MCP 서버 에러 핸들링 수정
- headless 세션에서 도구 사용성 수정
- AWS Bedrock의 모델 ID 유지 수정
- 레이트 리밋 에러 표시 수정
- 잘못된 형식의 텍스트 블록으로 인한 크래시 수정
- 좁은 터미널 높이에서 `/help` 렌더링 수정
- 키바인딩 검증 수정
- 비활성화된 플러그인의 설정 새로고침 수정
- SSH 16색 팔레트 표시 수정
- Bash 도구 권한 제안 수정
...
### 주요 하이라이트
- **`--bare` 플래그**: 스크립팅용 `-p` 호출 시 훅, LSP, 플러그인, 스킬 워크 등을 모두 건너뛰는 최소 실행 모드
- **`--channels` 권한 릴레이**: 채널 서버가 도구 승인 프롬프트를 폰으로 전달 가능
- **여러 동시 세션 OAuth 수정**: 여러 세션 실행 시 반복 재인증 문제 해결

### 새 기능
- **`--bare` 플래그**: 스크립팅 `-p` 호출 시 훅, LSP, 플러그인 동기화, 스킬 디렉토리 탐색을 건너뜀. `ANTHROPIC_API_KEY` 또는 `--settings`의 `apiKeyHelper` 필요 (OAuth/키체인 인증 비활성화), 자동 메모리도 완전 비활성화
- **`--channels` 권한 릴레이**: `permission` 기능을 선언한 채널 서버가 도구 승인 프롬프트를 폰으로 전달 가능
- **`claude remote-control` 서브커맨드**: 외부 빌드용 로컬 환경 서빙, 모든 사용자 사용 가능
- **플러그인 npm 레지스트리 지원**: 커스텀 npm 레지스트리 및 특정 버전 핀닝 지원
- **`CLAUDE_CODE_ACCOUNT_UUID`** 등 환경변수: SDK 호출자가 계정 정보를 동기적으로 제공, 초기 텔레메트리 레이스 컨디션 해소

### 개선
- **BashTool 로그인 셸 스킵**: 셸 스냅샷이 있으면 기본적으로 `-l` 플래그를 건너뛰어 실행 성능 향상 (기존 `CLAUDE_BASH_NO_LOGIN=true` 설정 불필요)
- **도구 결과 디스크 저장 임계값**: 50K 문자 이상의 도구 결과를 디스크에 저장 (기존 100K → 50K)하여 컨텍스트 윈도우 사용량 감소
- **`/model` 피커**: 핀된 모델 버전을 사람이 읽기 쉬운 레이블로 표시 (예: "Sonnet 4.5"), 새 버전 사용 가능 시 업그레이드 힌트
- **마켓플레이스 git 타임아웃**: 기본 30초 → 120초로 증가, `CLAUDE_CODE_PLUGIN_GIT_TIMEOUT_MS`로 설정 가능
- **`--resume` 메모리 사용량 68% 감소**: 세션 인덱스를 가벼운 stat 기반 로딩으로 교체
- **`TaskStop` 도구 개선**: 중지된 커맨드/태스크 설명을 결과에 표시
- **헤드리스 모드 성능**: `-p` 모드에서 시작 시 불필요한 API 호출 스킵
- **MCP 인증 실패 캐싱**: HTTP/SSE MCP 서버 인증 실패 시 캐싱하여 반복 시도 방지

### 버그 수정
- 여러 Claude Code 세션 동시 실행 시 하나가 OAuth 토큰을 갱신하면 반복 재인증이 필요하던 문제 수정
- 음성 모드가 재시도 실패를 조용히 삼키고 잘못된 "네트워크 확인" 메시지를 표시하던 문제 수정
- 음성 모드 오디오가 서버 WebSocket 연결 끊김 시 복구되지 않던 문제 수정
- `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS`가 `structured-outputs` 베타 헤더를 제거하지 않아 프록시 게이트웨이에서 400 에러 발생 문제 수정
- `--channels` 우회: 관리 설정 없는 Team/Enterprise 조직에서의 우회 문제 수정
- Node.js 18에서의 크래시 수정
- 문자열 내 대시가 있는 Bash 커맨드에서 불필요한 권한 프롬프트 문제 수정
- 플러그인 디렉토리가 세션 중 삭제될 때 플러그인 훅이 프롬프트 제출을 차단하던 문제 수정
- 백그라운드 에이전트 태스크 출력이 폴링 간격 사이에 태스크 완료 시 무한 대기하던 레이스 컨디션 수정
- 세션 재개 시 워크트리 자동 전환 지원
- `/btw`가 활성 응답 중 사용 시 붙여넣은 텍스트 미포함 문제 수정
- tmux에서 빠른 Cmd+Tab → 붙여넣기 시 클립보드 복사를 앞지를 수 있던 레이스 수정
- 백그라운드 작업 완료 시 터미널 탭 제목 미업데이트 문제 수정
- Remote Control 세션 제목 개선 및 `/rename` 동기화

### 변경
- **macOS**: `C:\ProgramData\ClaudeCode\managed-settings.json` 레거시 관리 설정 경로 제거 → `C:\Program Files\ClaudeCode\managed-settings.json` 사용
- **`SIMPLE` 모드**: MCP 도구, 첨부파일, 훅, CLAUDE.md 파일 로딩도 비활성화하여 완전 최소 경험 제공

### 플랫폼
- **[VSCode]** `/extra-usage` 명령 지원 추가
- **[VSCode]** 확장 시작 시 "command not found" 크래시 수정 (Windows)
...
### 주요 하이라이트
- **`rate_limits` 상태줄 필드**: Claude.ai 레이트 리밋 사용량(5시간/7일 윈도우) 표시
- **`effort` 프론트매터 지원**: 스킬/슬래시 커맨드에서 모델 노력 수준 오버라이드
- **`--channels` (리서치 프리뷰)**: MCP 서버가 세션에 메시지를 푸시 가능
- **관리 설정 핫 리로드**: `remote-settings.json` 캐시에서 누락되던 관리 설정 시작 시 적용 수정

### 새 기능
- **`rate_limits` 상태줄 필드**: 5시간 및 7일 윈도우의 `used_percentage`와 `resets_at` 포함
- **`source: 'settings'` 플러그인 소스**: settings.json에 플러그인 항목을 인라인으로 선언
- **CLI 도구 사용 감지**: 플러그인 팁에 파일 패턴 매칭 외에 CLI 도구 사용 감지 추가
- **`effort` 프론트매터**: 스킬/슬래시 커맨드 호출 시 모델 노력 수준 오버라이드
- **`--channels` (리서치 프리뷰)**: MCP 서버가 세션에 메시지를 푸시 가능
- **HTTP 훅**: 셸 커맨드 대신 URL에 JSON을 POST하고 JSON 응답 수신

### 개선
- **`@` 파일 자동완성**: 대규모 git 저장소에서 반응성 향상
- **`/effort`**: auto가 현재 어떤 수준으로 해석되는지 표시 (상태바 인디케이터와 일치)
- **`/permissions`**: Tab과 화살표 키로 리스트 내에서 탭 전환
- **백그라운드 태스크 패널**: 왼쪽 화살표로 리스트 뷰에서 닫기
- **플러그인 설치 팁 간소화**: 2단계 → 단일 `/plugin install` 명령
- **대규모 저장소 시작 메모리 감소**: ~80MB 절감 (250k 파일 기준)

### 버그 수정
- 음성 모드 WebSocket 실패: Cloudflare 봇 탐지로 인한 비브라우저 TLS 핑거프린트 문제 수정
- 세밀한 도구 스트리밍 사용 시 API 프록시, Bedrock, Vertex에서 400 에러 수정
- `/remote-control`이 게이트웨이/서드파티 프로바이더 배포에서 표시되던 문제 수정
- `/sandbox` 탭 전환이 Tab/화살표 키에 응답하지 않던 문제 수정
- 관리 설정이 시작 시 적용되지 않던 문제 수정 (`remote-settings.json` 캐시 문제)
