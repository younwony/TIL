...
모든 작업에 우선 적용되는 핵심 행동 가이드라인이다. (출처: Andrej Karpathy CLAUDE.md 핵심 흡수)

### 1. 가정은 명시하고 모호하면 멈춘다

- 여러 해석이 가능하면 silent하게 고르지 않는다. 선택지를 제시하고 묻는다.
- 확신이 없으면 가정을 명시적으로 표현한 뒤 진행한다.
- 더 단순한 접근이 있으면 말한다. 필요하면 푸시백한다.

### 2. 변경된 모든 라인은 사용자 요청에 직결되어야 한다

- diff 점검 시 "이 줄이 왜 필요한가" 답할 수 없으면 제거한다.
- 인접 코드, 주석, 포매팅을 "개선"하지 않는다. 매칭되는 기존 스타일을 따른다.
- 내 변경으로 새로 발생한 orphan(미사용 import/변수/함수)만 제거한다. 기존 dead code는 언급만 하고 손대지 않는다.

### 3. 성공 기준을 검증 가능한 형태로 변환한다

- "검증 추가" → "잘못된 입력에 대한 테스트 작성 후 통과시키기"
- "버그 수정" → "재현 테스트 작성 후 통과시키기"
- "리팩터링" → "변경 전후 테스트 통과 보장"
- 다단계 작업은 각 단계마다 verify 항목을 명시한다.
...
### 외부 AI 크로스 체크 timeout 정책 (필수)

work-plan, self-review, review-pr, team-review, 3ai-plan 등 Gemini/Codex를 호출하는 모든 skill·command에 적용:
